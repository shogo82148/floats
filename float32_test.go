...
func TestFloat32_Signbit(t *testing.T) {
	tests := []struct {
		...
		{Float32(math.Inf(1)), false},         // +Inf
		{Float32(math.Inf(-1)), true},         // -Inf
	}

	for _, tt := range tests {
		got := tt.in.Signbit()
		if got != tt.want {
			t.Errorf("Float32.Signbit() = %v, want %v", got, tt.want)
		}
	}
}

func BenchmarkFloat32_Signbit(b *testing.B) {
	f := Float32(1.0)
	for b.Loop() {
...
func TestFloat32_Uint128(t *testing.T) {
	tests := []struct {
		...
		{exact32(0), ints.Uint128{}},
		{exact32(1), ints.Uint128{0, 1}},
		{exact32(2), ints.Uint128{0, 2}},
		{exact32(1 << 63), ints.Uint128{0, 1 << 63}},
		{exact32(1 << 127), ints.Uint128{1 << 63, 0}},
	}

	for _, test := range tests {
		got := test.in.Uint128()
		if got != test.out {
			t.Errorf("Float32(%v).Uint128() = %v, want %v", test.in, got, test.out)
		}
	}
}
...
func TestFloat32_Int256(t *testing.T) {
	tests := []struct {
		...
		{exact32(0), ints.Int256{}},
		{exact32(1), ints.Int256{0, 0, 0, 1}},
		{exact32(2), ints.Int256{0, 0, 0, 2}},
		{exact32(1 << 63), ints.Int256{0, 0, 0, 1 << 63}},
		{exact32(1 << 127), ints.Int256{0, 0, 1 << (127 - 64), 0}},
		{
			exact32(-1),
			ints.Int256{ // -1
	...
	for _, test := range tests {
		got := test.in.Int256()
		if got != test.out {
			t.Errorf("Float32(%v).Int256() = %v, want %v", test.in, got, test.out)
		}
	}
}
...
func TestFloat32_Uint256(t *testing.T) {
	tests := []struct {
		...
		{exact32(0), ints.Uint256{}},
		{exact32(1), ints.Uint256{0, 0, 0, 1}},
		{exact32(2), ints.Uint256{0, 0, 0, 2}},
		{exact32(1 << 63), ints.Uint256{0, 0, 0, 1 << 63}},
		{exact32(1 << 127), ints.Uint256{0, 0, 1 << (127 - 64), 0}},
	}

	for _, test := range tests {
		got := test.in.Uint256()
		if got != test.out {
			t.Errorf("Float32(%v).Uint256() = %v, want %v", test.in, got, test.out)
		}
	}
}
