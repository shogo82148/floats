...
func (a Float128) Copysign(sign Float128) Float128 {
	return Float128{(a[0] &^ signMask128[0]) | (sign[0] & signMask128[0]), a[1]}
}
...
func (a Float128) Int256() ints.Int256 {
	sign, exp, frac := a.normalize()
	ret := ints.Int256{0, 0, frac[0], frac[1]}
	if exp <= shift128 {
		ret = ret.Rsh(uint(shift128 - exp))
	} else {
		ret = ret.Lsh(uint(exp - shift128))
	}
	if sign != 0 {
		ret = ret.Neg()
	}
...
func (a Float128) Uint256() ints.Uint256 {
	_, exp, frac := a.normalize()
	ret := ints.Uint256{0, 0, frac[0], frac[1]}
	if exp <= shift128 {
		ret = ret.Rsh(uint(shift128 - exp))
	} else {
		ret = ret.Lsh(uint(exp - shift128))
...
//	±Inf.Ldexp(exp) = ±Inf
//	NaN.Ldexp(exp) = NaN
func (a Float128) Ldexp(exp int) Float128 {
	// special cases
	if a.IsZero() || a.IsNaN() || a.IsInf(0) {
		return a
	}
	...
	sign, e, bits := a.normalize()
	e += exp
	if e >= mask128-bias128 {
		// overflow
		return Float128{sign | uvinf128[0], uvinf128[1]}
	}
	if e <= -bias128-shift128 {
		// underflow
		return Float128{sign, 0}
	}
	if e <= -bias128 {
		// the result is subnormal
		shift := -e - bias128 + 1
		frac := roundToNearestEven128(bits, uint(shift))
		return Float128{sign | frac[0], frac[1]}
	}
	bits[0] = sign | uint64(e+bias128)<<(shift128-64) | (bits[0] & fracMask128[0])
...
//	a.Mod(±Inf) = a
//	a.Mod(NaN) = NaN
func (a Float128) Mod(b Float128) Float128 {
	// special cases
