...
// NewFloat32Inf positive infinity if sign >= 0, negative infinity if sign < 0.
func NewFloat32Inf(sign int) Float32 {
	if sign >= 0 {
		return NewFloat32FromBits(uvinf32)
	}
	return NewFloat32FromBits(uvneginf32)
}
...
func (a Float32) Int128() ints.Int128 {
	sign, exp, frac := a.normalize()
	frac128 := ints.Int128{0, uint64(frac)}
	if exp <= shift32 {
		frac128 = frac128.Rsh(uint(shift32 - exp))
	} else {
		frac128 = frac128.Lsh(uint(exp - shift32))
	}
	if sign != 0 {
		frac128 = frac128.Neg()
	}
...
func (a Float32) Uint128() ints.Uint128 {
	_, exp, frac := a.normalize()
	frac128 := ints.Uint128{0, uint64(frac)}
	if exp <= shift32 {
		frac128 = frac128.Rsh(uint(shift32 - exp))
	} else {
		frac128 = frac128.Lsh(uint(exp - shift32))
...
func (a Float32) Int256() ints.Int256 {
	sign, exp, frac := a.normalize()
	frac256 := ints.Int256{0, 0, 0, uint64(frac)}
	if exp <= shift32 {
		frac256 = frac256.Rsh(uint(shift32 - exp))
	} else {
		frac256 = frac256.Lsh(uint(exp - shift32))
	}
	if sign != 0 {
		frac256 = frac256.Neg()
	}
...
//	±Inf.Ldexp(exp) = ±Inf
//	NaN.Ldexp(exp) = NaN
func (a Float32) Ldexp(exp int) Float32 {
