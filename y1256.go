...
// y1Asymptotic256 returns Y1(x) for x >= 500 using Hankel's asymptotic
// expansion
//
//	Y1(x) ~ sqrt(2/(pi*x)) * (P(x)*sin(x-3pi/4) + Q(x)*cos(x-3pi/4))
//	      = sqrt(1/(pi*x)) * ((Q(x)-P(x))*sin(x) - (P(x)+Q(x))*cos(x))
//
// using the same P/Q Hankel coefficients as j1Asymptotic256 (they depend
...
func y1Asymptotic256(x Float256) Float256 {
	var (
		...
		P28 = Float256{
			0xc00b_8237_9e68_5ace, 0xd31c_bcb5_3b07_5c02,
