...
func TestFloat128_SinLargeArgument(t *testing.T) {
	tests := []struct {
		x    Float128
		want string
	}{
		{exact128(math.Ldexp(1, 54)), "0.8973347529975925229804123768026911479052376676037185362023380651914704317378583068942725993960685025"},
		{exact128(math.Ldexp(1, 56)), "0.966999963061270720537678638626382199601190527737572484249243464830352176977790597110000034742920222"},
		{exact128(math.Ldexp(1, 60)), "-0.8306492176372546505752817955815103010761694744342921022530253811889396982714576378621020241394287786"},
		{exact128(math.Ldexp(1, 63)), "0.9999303766734422296178565053935299895486696117342805081232526661785769370875337024437183886526629467"},
	}
...
func TestFloat128_SinCosVeryLargeArgument(t *testing.T) {
	tests := []struct {
		x   string
		sin string
		cos string
		...
		{
			x:   "0x1p200",
			sin: "-0.4788977997069352278619903177234877673339103205414970159967429073433124696481192521336566659631149611",
			cos: "-0.8778706609950329973397280642564444440111377876129671109456226112684585912305247843610716893257446691",
		},
		{
			x:   "0x1p1000",
			sin: "-0.159201703086242438240048630820839033813686898777465015367510856943715371680677772898218532702189344",
			cos: "0.9872460775989134842399017963294680056270379668341074928488420389638803481261774084609784553775778973",
		},
		{
			x:   "0x1p8000",
			sin: "0.04037700624198965571944663499310739188559625586303762079853549656278166429957821916713116048008479035",
			cos: "-0.9991845161765339968240824361886456247828240569267463024133633705876403709709342018671646139902401231",
		},
		{
			x:   "-0x1p8000",
			sin: "-0.04037700624198965571944663499310739188559625586303762079853549656278166429957821916713116048008479035",
	...
	for _, tt := range tests {
		x, err := ParseFloat128(tt.x)
		if err != nil {
			t.Fatalf("ParseFloat128(%q) failed: %v", tt.x, err)
		}
		...
		if !close128(sin, tt.sin) {
			t.Errorf("Sin(%s) = %v; want %v", tt.x, sin, tt.sin)
		}
		if !close128(cos, tt.cos) {
			t.Errorf("Cos(%s) = %v; want %v", tt.x, cos, tt.cos)
		}
		if !eq128(sin, s2) || !eq128(cos, c2) {
			t.Errorf("Sincos(%s) mismatch: Sin=%v SincosSin=%v Cos=%v SincosCos=%v", tt.x, sin, s2, cos, c2)
		}
	}
}
